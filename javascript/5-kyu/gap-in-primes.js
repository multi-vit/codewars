...
function gap(gapSearch, startNumber, endNumber) {
  ...
  for (let i = startNumber; i <= endNumber; i++) {
    ...
    if (i > 1 && prime) {
      if (primeNumbers.length === 1) {
        if (i - primeNumbers[0] === gapSearch) {
          return [primeNumbers[0], i];
        } else {
          primeNumbers[0] = i;
        }
      } else {
...
/*
Need to know prime numbers from start to finish
Then need to compare digits in prime number sequence to see if gap exists
If so, return those 2 numbers
Else, return null
*/
